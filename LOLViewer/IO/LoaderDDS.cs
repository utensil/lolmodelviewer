    ...
    static class ImageDDS
    {
        ...
        public static void LoadFromDisk(string filename, out uint texturehandle, out TextureTarget dimension)
        {
            ...
            try // Exceptions will be thrown if any Problem occurs while working on the file. 
            {
                ...
                Load(_RawDataFromFile, out texturehandle, out dimension);
            }
            catch (Exception e)
            {
                dimension = (TextureTarget)0;
        ...
        public static void LoadFromMemory(byte[] data, out uint texturehandle, out TextureTarget dimension)
        {
            dimension = (TextureTarget)0;
            texturehandle = TextureLoaderParameters.OpenGLDefaultTexture;
            Load(data, out texturehandle, out dimension);
        }

        #region Helpers
        private static void Load(byte[] _RawDataFromFile, out uint texturehandle, out TextureTarget dimension)
        {
            dimension = (TextureTarget)0;
            texturehandle = TextureLoaderParameters.OpenGLDefaultTexture;
